...
test.describe('soft assertions', () => {
  test.skip('soft', async ({ page }) => {
    ...
    await expect.soft(page.locator('h1')).toHaveText('Example Domain1');  //fails but goes to next line
    await expect.soft(page.locator('p').first()).toBeVisible();  ////will run and pass because using soft in before line wont stop
    console.log("hello");
  });
});
